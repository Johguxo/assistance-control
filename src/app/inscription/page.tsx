...
export default function Home() {
  ...
  const alertSuccess = () => {
    const actualAge = calculateAge(formData.date_birth);
    let title = `<p>Deseas crear un nuevo asistente a la JAJ 😁</p>`
    let text = `<p>Recuerda que se tomara automaticamente la asistencia de hoy</p>`
    if (actualAge < 18) { 
      title =`<p>El usuario tiene <strong>${actualAge} años 🧐</strong>.</p> `
      text = `<p>Recuerda que el aceptando este registro estas validando que:</p>
      <ul>
          <li>✅ Tiene autorización de sus padres</li>
          <li>✅ Asistencia de hoy sábado</li>
      </ul>
      <p><strong>¿Deseas crearlo?</strong></p>`
    }
    MySwal.fire({
      title,
      html: text,
      ...
      if (result.isConfirmed) {
        ...
        return MySwal.fire(<p>Asistente creado satisfactoriamente</p>)
      }
    })
  }
  ...
  const calculateAge = (date_birth: string) => {
